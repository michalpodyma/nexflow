          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-12">
            ...
            <div className="lg:col-span-2">
              ...
              <div id="pracodawcy" className="mb-10">
                <h2 className="text-xl font-bold text-ink-inverse mb-4">{t("employers_heading")}</h2>
                ...
                <div className="mt-6 p-4 bg-white rounded-xl border border-gray-100">
                  ...
                  <div className="text-sm text-graphite space-y-1">
                    <p>
                      <a href="tel:+48224878828" className="hover:text-accent-dark transition-colors font-medium">
                        +48 224 878 828
                      </a>{" "}
                      <span className="text-slate">(PL/DE)</span>
                    </p>
                    <p className="text-slate mt-2">
                      <a href="mailto:info@nexflow.work" className="hover:text-accent-dark transition-colors">
                        info@nexflow.work
                      </a>
                    </p>
                  </div>
              ...
              {/* For workers */}
              <div id="pracownicy">
                <h2 className="text-xl font-bold text-ink-inverse mb-4">{t("workers_heading")}</h2>
                ...
                <div className="mb-4 p-4 bg-white rounded-xl border border-gray-100">
                  ...
                  <a href="tel:+48224878828" className="text-ink-inverse font-semibold hover:text-accent-dark transition-colors text-sm">
                    +48 224 878 828
                  </a>
                  <p className="text-xs text-slate mt-1">{t("hours")}</p>
                </div>
                <div className="p-4 bg-canvas rounded-xl text-white">
            ...
            <div className="lg:col-span-3">
              <div className="bg-white rounded-2xl p-8 border border-gray-100">
                <h2 className="text-xl font-bold text-ink-inverse mb-2">{t("form_heading")}</h2>
...
function ContactCard({ name, role, email }: { name: string; role: string; email: string }) {
  return (
    <div className="flex items-start gap-4 p-4 bg-white rounded-xl border border-gray-100">
      <div className="w-10 h-10 rounded-full bg-canvas flex items-center justify-center text-nexflow-cyan shrink-0 text-sm font-bold">
        {name.split(" ").map((n) => n[0]).join("")}
      </div>
      <div>
        <p className="font-semibold text-ink-inverse text-sm">{name}</p>
        <p className="text-slate text-xs mb-2">{role}</p>
        <p className="text-xs">
          <a href={`mailto:${email}`} className="text-ink-inverse hover:underline">{email}</a>
        </p>
      </div>
    </div>
  );
